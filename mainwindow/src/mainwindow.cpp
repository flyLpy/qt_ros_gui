...
void MainWindow::RecvChannelMsg(const MsgId &id, const std::any &data) {
  switch (id) {
    ...
    case MsgId::kImage: {
      ...
      this->SlotRecvImage(location_to_mat.first, std::make_shared<cv::Mat>(location_to_mat.second));
      sensor->setpicture3();
    } break;
    default:
      break;
...
MainWindow::~MainWindow() { delete ui;delete sensor; }
void MainWindow::setupUi() {
  ...
  ///////////////////////////////////////////////////////////////////////状态信息
  ads::CDockWidget* sensorDockWidget1 = new ads::CDockWidget("传感器状态");
  ...
  ////////////////////////////////////////////////////////////////////////建图
  QWidget *mapping_widget = new QWidget(this);
  QVBoxLayout *mapping_layout = new QVBoxLayout(mapping_widget);

  // 显示点云
  widgetOpenGL = new QVTKOpenGLNativeWidget(mapping_widget);
  widgetOpenGL->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
  mapping_layout->addWidget(widgetOpenGL);
  ...
  widgetOpenGL->setRenderWindow(viewer->getRenderWindow());
  viewer->setupInteractor(widgetOpenGL->interactor(), widgetOpenGL->renderWindow());
  cloud.reset(new pcl::PointCloud<pcl::PointXYZ>);

  // 创建加载按钮并连接槽函数
  loadButton = new QPushButton("加载点云地图", this);
  loadButton->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum);
  mapping_layout->addWidget(loadButton);
  connect(loadButton, &QPushButton::clicked, this, &MainWindow::loadAndShowPCD);

  // 创建按钮
  QPushButton *btn_start_mapping = new QPushButton("开始建图");
  QPushButton *btn_end_mapping = new QPushButton("结束建图");
  btn_start_mapping->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum);
  btn_end_mapping->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum);

  // 创建水平布局并添加按钮
  QHBoxLayout *horizontalLayout_mapping_1 = new QHBoxLayout();
  horizontalLayout_mapping_1->addWidget(btn_start_mapping);

  QHBoxLayout *horizontalLayout_mapping_2 = new QHBoxLayout();
  horizontalLayout_mapping_2->addWidget(btn_end_mapping);

  // 将水平布局添加到主垂直布局
  mapping_layout->addLayout(horizontalLayout_mapping_1);
  mapping_layout->addLayout(horizontalLayout_mapping_2);

  // 设置布局并添加到 dock widget
  mapping_widget->setLayout(mapping_layout);
  ads::CDockWidget *mapping_dock_widget = new ads::CDockWidget("建图");
  mapping_dock_widget->setWidget(mapping_widget);
  mapping_dock_widget->setMinimumSizeHintMode(CDockWidget::MinimumSizeHintFromDockWidget);
  mapping_dock_widget->setMinimumSize(200, 150);
  mapping_dock_widget->setMaximumSize(QWIDGETSIZE_MAX, QWIDGETSIZE_MAX);

  // 将 "建图" 窗口添加到右侧区域，与 "Task" 窗口并列
  dock_manager_->addDockWidget(ads::DockWidgetArea::RightDockWidgetArea, mapping_dock_widget, center_docker_area_);
  mapping_dock_widget->toggleView(false);

  // 为 "建图" 窗口添加一个 toggle action 到视图菜单
  ui->menuView->addAction(mapping_dock_widget->toggleViewAction());
  connect(btn_start_mapping, &QPushButton::clicked, this, &MainWindow::begin_mapping);
  connect(btn_end_mapping, &QPushButton::clicked, this, &MainWindow::end_mapping);
  ...
  QVBoxLayout *verticalLayout_13 = new QVBoxLayout();
  nav_goal_table_view_->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
  verticalLayout_13->addWidget(nav_goal_table_view_);
  task_list_widget->setLayout(verticalLayout_13);

  ads::CDockWidget *nav_goal_list_dock_widget = new ads::CDockWidget("导航");
  ...
  // 设置按钮和复选框的尺寸策略
  btn_add_one_goal->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum);
  btn_start_task_chain->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum);
  loop_task_checkbox->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum);
  btn_load_task_chain->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum);
  btn_save_task_chain->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum);
  ...
  nav_goal_list_dock_widget->setMaximumSize(QWIDGETSIZE_MAX, QWIDGETSIZE_MAX);

  // 将导航目标列表 Dock Widget 添加到右侧区域
  ...
  // 为导航目标列表 Dock Widget 添加一个 toggle action 到视图菜单
...
/////////////////////////////////////////////////////////////////////////////建图的槽函数
void MainWindow::begin_mapping(){

}
void MainWindow::end_mapping(){

}

void MainWindow::loadAndShowPCD()
{
    std::cout << "Attempting to load PCD file..." << std::endl;

    // 加载点云文件
    if (pcl::io::loadPCDFile("/home/lpy/0DATA/下载/map.pcd", *cloud) == -1)
    {
        // 处理加载失败的情况
    ...
    // 强制渲染窗口重绘
    widgetOpenGL->renderWindow()->Render();
    widgetOpenGL->repaint();
}
